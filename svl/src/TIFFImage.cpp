...
TIFFImage::TIFFImage() : baseImage(), _tiff(NULL), _jp2000(NULL) {
}
...
void* TIFFImage::readDataFromImage(const long long& startX, const long long& startY, const unsigned long long& width, 
  ...
  if (getDataType()==UInt32) {
    unsigned int* temp = FillRequestedRegionFromTIFF<unsigned int>(startX, startY, width, height,  _samplesPerPixel);
    return (void*)temp;
  }
  else if (getDataType()==UInt16) {
    unsigned short* temp = FillRequestedRegionFromTIFF<unsigned short>(startX, startY, width, height,  _samplesPerPixel);
    return (void*)temp;
  }
  else if (getDataType()==Float) {
    float* temp = FillRequestedRegionFromTIFF<float>(startX, startY, width, height,  _samplesPerPixel);
    return (void*)temp;
  }
  else if (getDataType()==UChar) {
    unsigned char* temp = FillRequestedRegionFromTIFF<unsigned char>(startX, startY, width, height,  _samplesPerPixel);
    return (void*)temp;
  }
  else {
    return NULL;
...
{
  std::lock_guard <std::mutex> l(*_openCloseMutex);
  T* temp = new T[width*height*nrSamples];
  std::fill(temp,temp+width*height*nrSamples,0);
  unsigned int tileW=_tileSizesPerLevel[0][0], tileH=_tileSizesPerLevel[0][1], levelH=_levelDimensions[0][1], levelW=_levelDimensions[0][0];

    long long levelStartX = startX;
    long long levelStartY = startY;
