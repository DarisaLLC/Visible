...
const char lifIO::LIF_MAGIC_BYTE = 0x70;
const char lifIO::LIF_MEMORY_BYTE = 0x2a;

lifIO::lifIO() :  _lastChannel(0), _alternateCenter(false), _selectedSeries(-1), _fileSize(0), _fileName("") {
}
...
bool lifIO::initialize(const std::string& imagePath) {
    cleanup();
...
void lifIO::translateMetaData(pugi::xml_document& doc) {
    ...
    _colorTypes = std::vector<int>(images.size(), -1);
    _dataTypes = std::vector<int>(images.size(), -1);
    ...
    std::vector<int> _newColorTypes;
    std::vector<int> _newDataTypes;
...
int lifIO::getTileIndex(int index) {
    int count = 0;
...
void*lifIO::readDataFromImage(const long long& startX, const long long& startY, const unsigned long long& width,
...
void lifIO::translateImageNames(pugi::xpath_node& imageNode, int imageNr) {
...
void lifIO::translateAttachmentNodes(pugi::xpath_node&  imageNode, int imageNr)
{
...
void lifIO::translateImageNodes(pugi::xpath_node&  imageNode, int imageNr)
{
