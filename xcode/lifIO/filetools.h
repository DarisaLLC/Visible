...
// and was successfully deleted.
  bool deleteDir(const std::string &name, bool deleteNonEmpty = false);
...
//////////////////////////
  ...
  */
  std::string getPathRelativeToLocation(const std::string &pathToAlter, const std::string &fixedPath);
...
// Returns true iff file exists and was successfully read.
  ...
  bool readFileTail(const std::string &filename, std::vector<std::string> &vs, int nBytesToRead = 2048);
...
// (un)available on a windows computer.
  ...
  std::string stripTrailingSlash(std::string str);
