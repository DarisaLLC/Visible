...
- (void)thirdButtonWasTapped:(id)sender;
...
@synthesize thirdButtonCallback = _thirdButtonCallback;
...
- (void)addCinderViewToFront
{
	...
	cinderViewParent.title = @" Capture Proof ";
...
- (NSArray *)tabBarItems
{
	...
	UIBarButtonItem *button3 = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemSearch target:self action: @selector(thirdButtonWasTapped:)];
...
- (void)thirdButtonWasTapped:(id)sender
{
    if( _thirdButtonCallback ) {
        _thirdButtonCallback() ;
    }
}
