...
void skinCheckApp::setup()
{
    ...
    [sNativeController setInfoButtonCallback: bind( &skinCheckApp::infoTapped, this ) ];
    [sNativeController setThirdButtonCallback: bind( &skinCheckApp::thirdTapped, this ) ];
}


void skinCheckApp::thirdTapped()
{
    mFocusStepMode = ! mFocusStepMode;
}

void skinCheckApp::infoTapped()
{
...
void skinCheckApp::setupVisuals()
{
    printDevices();
    float x_offset = 0.92;
    float y_offset = 0.25;
...
void skinCheckApp::configureFrameRate ()
{
    ...
    AVCaptureDevice* avd = (__bridge AVCaptureDevice*)device->getNative();
        ...
        for ( AVCaptureDeviceFormat * format in [avd formats] ) {
            for ( AVFrameRateRange * range in format.videoSupportedFrameRateRanges ) {
                if ( range.maxFrameRate > bestFrameRateRange.maxFrameRate ) {
                    bestFormat = format;
                    bestFrameRateRange = range;
                    std::cout << "max fps" << range.maxFrameRate << std::endl;
                }
            }
        }
        if ( bestFormat ) {
            if ( [avd lockForConfiguration:NULL] == YES ) {
                avd.activeFormat = bestFormat;
                avd.activeVideoMinFrameDuration = bestFrameRateRange.minFrameDuration;
                avd.activeVideoMaxFrameDuration = bestFrameRateRange.maxFrameDuration;
                [avd unlockForConfiguration];
            }
        }
}

bool skinCheckApp::focus_at (float lens_location)
{
...
int skinCheckApp::focus_mode () const
{
    auto device = mCapture->getDevice();
    AVCaptureDevice* avd = (__bridge AVCaptureDevice*)device->getNative();
...
bool skinCheckApp::focus_mode (int new_mode)
{
    auto device = mCapture->getDevice();
    AVCaptureDevice* avd = (__bridge AVCaptureDevice*)device->getNative();
    NSError *error = nil;
...
void skinCheckApp::captureThreadFn(gl::ContextRef context)
{
   	...
   	while(! mShouldQuit )
    {
        ...
        if( mCapture && mCapture->checkNewFrame() )
        {
            ...
            mCount++;
            mFps = mCount / (getElapsedSeconds() - mStartTime);
        }
    }
...
CINDER_APP( skinCheckApp, RendererGl, skinCheckApp::prepareSettings  );
