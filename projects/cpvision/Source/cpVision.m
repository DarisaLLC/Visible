...
NSString * const cpVisionErrorDomain = @"cpVisionErrorDomain";

static uint64_t const cpVisionRequiredMinimumDiskSpaceInBytes = 49999872; // ~ 47 MB
static CGFloat const cpVisionThumbnailWidth = 160.0f;

// KVO contexts

static NSString * const cpVisionFocusObserverContext = @"cpVisionFocusObserverContext";
static NSString * const cpVisionExposureObserverContext = @"cpVisionExposureObserverContext";
static NSString * const cpVisionWhiteBalanceObserverContext = @"cpVisionWhiteBalanceObserverContext";
static NSString * const cpVisionFlashModeObserverContext = @"cpVisionFlashModeObserverContext";
static NSString * const cpVisionTorchModeObserverContext = @"cpVisionTorchModeObserverContext";
static NSString * const cpVisionFlashAvailabilityObserverContext = @"cpVisionFlashAvailabilityObserverContext";
static NSString * const cpVisionTorchAvailabilityObserverContext = @"cpVisionTorchAvailabilityObserverContext";
static NSString * const cpVisionCaptureStillImageIsCapturingStillImageObserverContext = @"cpVisionCaptureStillImageIsCapturingStillImageObserverContext";

// additional video capture keys

NSString * const cpVisionVideoRotation = @"cpVisionVideoRotation";

// photo dictionary key definitions

NSString * const cpVisionPhotoMetadataKey = @"cpVisionPhotoMetadataKey";
NSString * const cpVisionPhotoJPEGKey = @"cpVisionPhotoJPEGKey";
NSString * const cpVisionPhotoImageKey = @"cpVisionPhotoImageKey";
NSString * const cpVisionPhotoThumbnailKey = @"cpVisionPhotoThumbnailKey";

// video dictionary key definitions

NSString * const cpVisionVideoPathKey = @"cpVisionVideoPathKey";
NSString * const cpVisionVideoThumbnailKey = @"cpVisionVideoThumbnailKey";
NSString * const cpVisionVideoThumbnailArrayKey = @"cpVisionVideoThumbnailArrayKey";
NSString * const cpVisionVideoCapturedDurationKey = @"cpVisionVideoCapturedDurationKey";

// cpGLProgram shader uniforms for pixel format conversion on the GPU
typedef NS_ENUM(GLint, cpVisionUniformLocationTypes)
{
    cpVisionUniformY,
    cpVisionUniformUV,
    cpVisionUniformCount
};
    ...
    cpMediaWriterDelegate>
{
    ...
    cpMediaWriter *_mediaWriter;

    dispatch_queue_t _captureSessionDispatchQueue;
    dispatch_queue_t _captureCaptureDispatchQueue;

    cpCameraDevice _cameraDevice;
    cpCameraMode _cameraMode;
    cpCameraOrientation _cameraOrientation;

    cpCameraOrientation _previewOrientation;
    BOOL _autoUpdatePreviewOrientation;
    BOOL _autoFreezePreviewDuringCapture;
    BOOL _usesApplicationAudioSession;

    cpFocusMode _focusMode;
    cpExposureMode _exposureMode;
    cpFlashMode _flashMode;
    cpMirroringMode _mirroringMode;

    NSString *_captureSessionPreset;
    NSString *_captureDirectory;
    cpOutputFormat _outputFormat;
    ...
    cpCameraDevice _bufferDevice;
    cpCameraOrientation _bufferOrientation;
    size_t _bufferWidth;
    size_t _bufferHeight;
    CGRect _presentationFrame;

    EAGLContext *_context;
    cpGLProgram *_program;
...
{
    static cpVision *singleton = nil;
    static dispatch_once_t once = 0;
    dispatch_once(&once, ^{
        singleton = [[cpVision alloc] init];
    });
    return singleton;
}
...
- (void)setCameraOrientation:(cpCameraOrientation)cameraOrientation
{
...
- (void)setPreviewOrientation:(cpCameraOrientation)previewOrientation {
...
- (void)_setOrientationForConnection:(AVCaptureConnection *)connection
{
    ...
    switch (_cameraOrientation) {
        case cpCameraOrientationPortraitUpsideDown:
            orientation = AVCaptureVideoOrientationPortraitUpsideDown;
            break;
        case cpCameraOrientationLandscapeRight:
            orientation = AVCaptureVideoOrientationLandscapeRight;
            break;
        case cpCameraOrientationLandscapeLeft:
            orientation = AVCaptureVideoOrientationLandscapeLeft;
            break;
        case cpCameraOrientationPortrait:
        default:
            break;
    }
...
- (void)_setCameraMode:(cpCameraMode)cameraMode cameraDevice:(cpCameraDevice)cameraDevice outputFormat:(cpOutputFormat)outputFormat
{
...
- (void)setCameraDevice:(cpCameraDevice)cameraDevice
{
...
- (void)setCameraMode:(cpCameraMode)cameraMode
{
    [self _setCameraMode:cameraMode cameraDevice:_cameraDevice outputFormat:_outputFormat];
}

- (void)setOutputFormat:(cpOutputFormat)outputFormat
{
...
- (void)setFocusMode:(cpFocusMode)focusMode
{
...
- (void)setExposureMode:(cpExposureMode)exposureMode
{
...
- (void) _setCurrentDevice:(AVCaptureDevice *)device
{
    ...
    _exposureMode   = (cpExposureMode)device.exposureMode;
    _focusMode      = (cpFocusMode)device.focusMode;
}
...
- (void)setFlashMode:(cpFlashMode)flashMode
{
...
- (BOOL)supportsVideoFrameRate:(NSInteger)videoFrameRate
{
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1) {
        ...
        if (self.cameraDevice == cpCameraDeviceBack) {
...
- (id)init
{
    ...
    if (self) {
        ...
        _videoBitRate = cpVideoBitRate640x480;
        ...
        // setup queues
        _captureSessionDispatchQueue = dispatch_queue_create("cpVisionSession", DISPATCH_QUEUE_SERIAL); // protects session
        _captureCaptureDispatchQueue = dispatch_queue_create("cpVisionCapture", DISPATCH_QUEUE_SERIAL); // protects capture
        
        _previewLayer = [[AVCaptureVideoPreviewLayer alloc] initWithSession:nil];
        
        _maximumCaptureDuration = kCMTimeInvalid;

        [self setMirroringMode:cpMirroringAuto];
...
typedef void (^cpVisionBlock)();

- (void)_enqueueBlockOnCaptureSessionQueue:(cpVisionBlock)block
{
...
- (void)_enqueueBlockOnCaptureVideoQueue:(cpVisionBlock)block
{
...
- (void)_enqueueBlockOnMainQueue:(cpVisionBlock)block
{
...
- (void)_executeBlockOnMainQueue:(cpVisionBlock)block
{
...
- (void)_commitBlock:(cpVisionBlock)block
{
...
- (void)_setupCamera
{
    ...
    // capture devices
    _captureDeviceFront = [cpVisionUtilities captureDeviceForPosition:AVCaptureDevicePositionFront];
    _captureDeviceBack = [cpVisionUtilities captureDeviceForPosition:AVCaptureDevicePositionBack];
    ...
    if (_cameraMode != cpCameraModePhoto && _flags.audioCaptureEnabled) {
    ...
    [self addObserver:self forKeyPath:@"currentDevice.adjustingFocus" options:NSKeyValueObservingOptionNew context:(__bridge void *)cpVisionFocusObserverContext];
    [self addObserver:self forKeyPath:@"currentDevice.adjustingExposure" options:NSKeyValueObservingOptionNew context:(__bridge void *)cpVisionExposureObserverContext];
    [self addObserver:self forKeyPath:@"currentDevice.adjustingWhiteBalance" options:NSKeyValueObservingOptionNew context:(__bridge void *)cpVisionWhiteBalanceObserverContext];
    [self addObserver:self forKeyPath:@"currentDevice.flashMode" options:NSKeyValueObservingOptionNew context:(__bridge void *)cpVisionFlashModeObserverContext];
    [self addObserver:self forKeyPath:@"currentDevice.torchMode" options:NSKeyValueObservingOptionNew context:(__bridge void *)cpVisionTorchModeObserverContext];
    [self addObserver:self forKeyPath:@"currentDevice.flashAvailable" options:NSKeyValueObservingOptionNew context:(__bridge void *)cpVisionFlashAvailabilityObserverContext];
    [self addObserver:self forKeyPath:@"currentDevice.torchAvailable" options:NSKeyValueObservingOptionNew context:(__bridge void *)cpVisionTorchAvailabilityObserverContext];

    // KVO is only used to monitor focus and capture events
    [_captureOutputPhoto addObserver:self forKeyPath:@"capturingStillImage" options:NSKeyValueObservingOptionNew context:(__bridge void *)(cpVisionCaptureStillImageIsCapturingStillImageObserverContext)];
...
- (void)_setupSession
{
    ...
    BOOL shouldSwitchDevice = (_currentDevice == nil) ||
                              ((_currentDevice == _captureDeviceFront) && (_cameraDevice != cpCameraDeviceFront)) ||
                              ((_currentDevice == _captureDeviceBack) && (_cameraDevice != cpCameraDeviceBack));
    
    BOOL shouldSwitchMode = (_currentOutput == nil) ||
                            ((_currentOutput == _captureOutputPhoto) && (_cameraMode != cpCameraModePhoto)) ||
                            ((_currentOutput == _captureOutputVideo) && (_cameraMode != cpCameraModeVideo));
    ...
    if (shouldSwitchMode) {
    	...
    	if (self.cameraMode == cpCameraModePhoto) {
...
- (void)setMirroringMode:(cpMirroringMode)mirroringMode
{
    ...
    switch (_mirroringMode) {
        ...
        case cpMirroringAuto:
        default:
		{
			if ([videoConnection isVideoMirroringSupported]) {
                BOOL mirror = (_cameraDevice == cpCameraDeviceFront);
...
{
    BOOL isDiskSpaceAvailable = [cpVisionUtilities availableDiskSpaceInBytes] > cpVisionRequiredMinimumDiskSpaceInBytes;
    ...
    if (provider) {
        ...
        if (imageSource) {
            if (CGImageSourceGetCount(imageSource) > 0) {
                ...
                options[(id)kCGImageSourceThumbnailMaxPixelSize] = @(cpVisionThumbnailWidth);
...
- (void)_capturePhotoFromSampleBuffer:(CMSampleBufferRef)sampleBuffer
{
    ...
    NSDictionary *tiffDict = @{ (NSString *)kCGImagePropertyTIFFSoftware : @"cpVision",
                                    (NSString *)kCGImagePropertyTIFFDateTime : [NSString cpformattedTimestampStringFromDate:[NSDate date]] };
    ...
    if (metadata) {
        photoDict[cpVisionPhotoMetadataKey] = metadata;
    ...
    if (uiImage) {
        if (_outputFormat == cpOutputFormatSquare) {
            uiImage = [self _squareImageWithImage:uiImage scaledToSize:uiImage.size];
        }
        // cpOutputFormatWidescreen
        // cpOutputFormatStandard
    
        photoDict[cpVisionPhotoImageKey] = uiImage;
        
        // add JPEG, thumbnail
        NSData *jpegData = UIImageJPEGRepresentation(uiImage, 0);
        if (jpegData) {
            // add JPEG
            photoDict[cpVisionPhotoJPEGKey] = jpegData;
            
            // add thumbnail
            if (_flags.thumbnailEnabled) {
                UIImage *thumbnail = [self _thumbnailJPEGData:jpegData];
                if (thumbnail) {
                    photoDict[cpVisionPhotoThumbnailKey] = thumbnail;
                }
            }
        }
    } else {
        DLog(@"failed to create image from JPEG");
        error = [NSError errorWithDomain:cpVisionErrorDomain code:cpVisionErrorCaptureFailed userInfo:nil];
    }
...
- (void)capturePhoto
{
    if (![self _canSessionCaptureWithOutput:_currentOutput] || _cameraMode != cpCameraModePhoto) {
        [self _failPhotoCaptureWithErrorCode:cpVisionErrorSessionFailed];
    ...
    [_captureOutputPhoto captureStillImageAsynchronouslyFromConnection:connection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        if (!imageDataSampleBuffer) {
            [self _failPhotoCaptureWithErrorCode:cpVisionErrorCaptureFailed];
        ...
        if (jpegData) {
            ...
            if (image) {
                photoDict[cpVisionPhotoImageKey] = image;
            } else {
                DLog(@"failed to create image from JPEG");
                error = [NSError errorWithDomain:cpVisionErrorDomain code:cpVisionErrorCaptureFailed userInfo:nil];
            }
...
{
    if (![self _canSessionCaptureWithOutput:_currentOutput] || _cameraMode != cpCameraModeVideo) {
        [self _failVideoCaptureWithErrorCode:cpVisionErrorSessionFailed];
        DLog(@"session is not setup properly for capture");
        return;
    }
    ...
    [self _enqueueBlockOnCaptureVideoQueue:^{
        ...
        if ([_delegate respondsToSelector:@selector(vision:willStartVideoCaptureToFile:)]) {
            ...
            if (!outputFile) {
                [self _failVideoCaptureWithErrorCode:cpVisionErrorBadOutputFile];
                return;
            }
        }
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:outputPath]) {
            ...
            if (![[NSFileManager defaultManager] removeItemAtPath:outputPath error:&error]) {
                [self _failVideoCaptureWithErrorCode:cpVisionErrorOutputFileExists];
        ...
        _mediaWriter = [[cpMediaWriter alloc] initWithOutputURL:outputURL];
    ...
    [self _enqueueBlockOnCaptureVideoQueue:^{
        ...
        void (^finishWritingCompletionHandler)(void) = ^{
            ...
            [self _enqueueBlockOnMainQueue:^{
                ...
                if (path) {
                    videoDict[cpVisionVideoPathKey] = path;
                ...
                videoDict[cpVisionVideoCapturedDurationKey] = @(capturedDuration);
...
- (void)cancelVideoCapture
{
    ...
    [self _enqueueBlockOnCaptureVideoQueue:^{
        ...
        void (^finishWritingCompletionHandler)(void) = ^{
            ...
            [self _enqueueBlockOnMainQueue:^{
                NSError *error = [NSError errorWithDomain:cpVisionErrorDomain code:cpVisionErrorCancelled userInfo:nil];
                if ([_delegate respondsToSelector:@selector(vision:capturedVideo:error:)]) {
...
#pragma clang diagnostic pop
    ...
    if (defaultThumbnail) {
        [videoDict setObject:defaultThumbnail forKey:cpVisionVideoThumbnailKey];
    }
    
    if (thumbnails.count) {
        [videoDict setObject:thumbnails forKey:cpVisionVideoThumbnailArrayKey];
    }
}
...
- (void)_failVideoCaptureWithErrorCode:(NSInteger)errorCode
{
    if (errorCode && [_delegate respondsToSelector:@selector(vision:capturedVideo:error:)]) {
        NSError *error = [NSError errorWithDomain:cpVisionErrorDomain code:errorCode userInfo:nil];
        [_delegate vision:self capturedVideo:nil error:error];
    }
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!_flags.recording || _flags.paused) {
           NSLog(@"CPV:2189");
        // process the sample buffer for rendering overlay layer or capturing video photo
            [self _executeBlockOnMainQueue:^{
    ...
    if (CMTIME_IS_VALID(_timeOffset)) {
        ...
        bufferToWrite = [cpVisionUtilities createOffsetSampleBufferWithSampleBuffer:sampleBuffer withTimeOffset:_timeOffset];
        if (!bufferToWrite) {
...
{
    if ( context == (__bridge void *)cpVisionFocusObserverContext ) {
    ...
    else if ( context == (__bridge void *)cpVisionExposureObserverContext ) {
    ...
    else if ( context == (__bridge void *)cpVisionWhiteBalanceObserverContext ) {
    ...
    else if ( context == (__bridge void *)cpVisionFlashAvailabilityObserverContext ||
              context == (__bridge void *)cpVisionTorchAvailabilityObserverContext ) {
    ...
    else if ( context == (__bridge void *)cpVisionFlashModeObserverContext ||
              context == (__bridge void *)cpVisionTorchModeObserverContext ) {
    ...
    else if ( context == (__bridge void *)cpVisionCaptureStillImageIsCapturingStillImageObserverContext ) {
...
#pragma mark - cpMediaWriterDelegate

- (void)mediaWriterDidObserveAudioAuthorizationStatusDenied:(cpMediaWriter *)mediaWriter
{
    [self _enqueueBlockOnMainQueue:^{
        [_delegate visionDidChangeAuthorizationStatus:cpAuthorizationStatusAudioDenied];
    }];
}
...
//    };
    ...
    GLuint vertexAttributeLocation = [_program attributeLocation:cpGLProgramAttributeVertex];
    GLuint textureAttributeLocation = [_program attributeLocation:cpGLProgramAttributeTextureCoord];
    
    glEnableVertexAttribArray(vertexAttributeLocation);
    glVertexAttribPointer(vertexAttributeLocation, 2, GL_FLOAT, GL_FALSE, 0, vertices);
    
    if (_cameraDevice == cpCameraDeviceFront) {
...
- (void)_setupGL
{
    static GLint uniforms[cpVisionUniformCount];

    [EAGLContext setCurrentContext:_context];
    
    NSBundle *bundle = [NSBundle bundleForClass:[cpVision class]];
    ...
    _program = [[cpGLProgram alloc] initWithVertexShaderName:vertShaderName fragmentShaderName:fragShaderName];
    [_program addAttribute:cpGLProgramAttributeVertex];
    [_program addAttribute:cpGLProgramAttributeTextureCoord];
    [_program link];
    
    uniforms[cpVisionUniformY] = [_program uniformLocation:@"u_samplerY"];
    uniforms[cpVisionUniformUV] = [_program uniformLocation:@"u_samplerUV"];
    [_program use];
            
    glUniform1i(uniforms[cpVisionUniformY], 0);
    glUniform1i(uniforms[cpVisionUniformUV], 1);
}
