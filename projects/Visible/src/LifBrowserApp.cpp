...
class LifBrowserApp : public App {
  public:
	void setup() override;
...
void LifBrowserApp::createItem( const serie_info &line, int lineNumber )
{
    string title				= line.name;
    string desc					= " Channels: " + to_string(line.channelCount) + "   width: " + to_string(line.dimensions[0]) +
