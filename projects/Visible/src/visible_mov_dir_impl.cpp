...
void movDirContext::setup()
{
    ...
    m_Dm = DirMovie::create (source_path(), m_extension, m_fps, m_anonymous_format);
...
{
    std::shared_ptr<uContext> mw(new movDirContext(createWindow( Window::Format().size(mMovieDisplayRect.getSize()))));
