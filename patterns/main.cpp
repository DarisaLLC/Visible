...
{
    std::string filename = "Calibration_1461000001_left.psm";
    ...
    if (res.second)
    {
        ...
        
        {
            ...
            double endtime = (std::clock() - start) / ((double)CLOCKS_PER_SEC);
            std::cout << " 3x3 Gaussian  " << endtime * scale << " millieseconds per " << std::endl;
        }
...
#if 0
        size_t sum = 0;
        std::vector<size_t> dhist;
        for (int i = 0; i < 256; i++)
        {
            dhist.push_back(output[i].size());
            sum += dhist.back();
        }
        for (int i = 0; i < 256; i++)
            std::cout << dhist[i] << std::endl;
        
        std::cout << sum << std::endl;
#endif
        
        showImageAndResult(smooth, output, db);
    }
}
...
int main(int argc, char ** argv)
{
    ...
    fs::path projectpath = installpath.parent_path();
    projectpath = projectpath.parent_path();
    projectpath = projectpath.parent_path();
    fs::path assetpath = projectpath.string() + "/assets/";
