...
template <typename T>
class root : public refcounted_ptr<root<T> >
{
public:
    ...
    // Constructors
    root(image_memory_alignment_policy imap = align_every_row, int alignment_bytes = 16)
        : m_align_policy(imap), _bayer_type(NoneBayer), m_align(alignment_bytes)

    {
    }

    root(int32_t width, int32_t height, image_memory_alignment_policy imap = align_every_row, int alignment_bytes = 16)
        : m_align_policy(imap), _bayer_type(NoneBayer), m_align(alignment_bytes)
    {
    ...
    root(uint8_t * rawPixels, int32_t RowUpdateBytes, int32_t awidth, int32_t aheight, image_memory_alignment_policy imap = align_every_row, int alignment_bytes = 16)
        : m_align_policy(imap), _bayer_type(NoneBayer), m_align(alignment_bytes)
    {
        assert(aheight > 0);
        assert(awidth > 0);
    ...
    {
        return (pixel_ptr_t)alignedPixelData() + y * rowPixelUpdate();
    }
    ...
    // Row update constant: constant address to add to next pixel next row
    int32_t rowPixelUpdate() const { return rowUpdate() / T::bytes(); }
