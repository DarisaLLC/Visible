...
namespace ImGui {
		...
		const std::vector<std::string>& extensionsAllowed)
	{
		...
		} else if (ImGui::BeginPopupModal(name.c_str(), NULL, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			// Subdirectories list.
			ImGui::PushItemWidth(scaling * 200);
            const bool clickedDir = ImGui::ListBox("##SelectFileBoxSubdirectories", &filePickerInternals.currentSubdirId, filePickerInternals.rawDirs.empty() ? NULL : &filePickerInternals.rawDirs[0], static_cast<int>(filePickerInternals.rawDirs.size()), 15);
			ImGui::PopItemWidth();
			ImGui::SameLine();
			// Files list.
			ImGui::PushItemWidth(scaling * 300);
			const bool clickedFile = ImGui::ListBox("##SelectFileBoxFiles", &filePickerInternals.currentFileId, filePickerInternals.rawItems.empty() ? NULL : &filePickerInternals.rawItems[0], static_cast<int>(filePickerInternals.rawItems.size()), 15);
			ImGui::PopItemWidth();
