...
struct console::NameArguments<T, Args...>
{
	...
	{
		std::string nameT;
		if(boost::typeindex::type_id<T>() == boost::typeindex::type_id<int>())
		{
			nameT = "<int";
		}
		else if(boost::typeindex::type_id<T>() == boost::typeindex::type_id<float>())
		{
			nameT = "<float";
		}
		else if(boost::typeindex::type_id<T>() == boost::typeindex::type_id<std::string>())
		{
			nameT = "<string";
		}
		else
